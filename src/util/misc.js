...
function rotatePoint(point, origin, radians) {
  var newPoint = new Point(point.x - origin.x, point.y - origin.y),
      v = rotateVector(newPoint, radians);
 ...
 */
function calcAngleBetweenVectors(a, b) {
 ...
 */
function projectStrokeOnPoints(points, options, openPath) {
  ...
  points.forEach(function (p, index) {
    ...
    if (index === 0) {
      C = points[index + 1];
      B = openPath ? getStrokeHatVector(createVector(C, A)).addEquals(A) : points[points.length - 1];
    }
    else if (index === points.length - 1) {
      B = points[index - 1];
      C = openPath ? getStrokeHatVector(createVector(B, A)).addEquals(A) : points[0];
    }
    else {
      B = points[index - 1];
      C = points[index + 1];
    }
    var bisector = getBisector(A, B, C),
 ...
 * @returns {Point} transformed point
 */
function transformPointRelativeToCanvas(point, canvas, relationBefore, relationAfter) {
 ...
 */
function makeBoundingBoxFromPoints(points, transform) {
  if (transform) {
  ...
  var xPoints = [points[0].x, points[1].x, points[2].x, points[3].x],
      minX = array.min(xPoints),
      maxX = array.max(xPoints),
      width = maxX - minX,
      yPoints = [points[0].y, points[1].y, points[2].y, points[3].y],
      minY = array.min(yPoints),
      maxY = array.max(yPoints),
      height = maxY - minY;
...
function parseUnit(value, fontSize) {
  ...
  if (!fontSize) {
    fontSize = Text.DEFAULT_SVG_FONT_SIZE;
  }
  switch (unit[0]) {
    case 'mm':
...
function getKlass(type, namespace) {
  ...
  type = string.camelize(type.charAt(0).toUpperCase() + type.slice(1));
 ...
 */
function getSvgAttributes(type) {
  var attributes = [
    'instantiated_by_use',
    'style',
    'id',
 ...
 */
function enlivenObjects(objects, namespace, reviver) {
  return Promise.all(objects.map(function(obj) {
    var klass = getKlass(obj.type, namespace);
...
function enlivenObjectEnlivables(serializedObject) {
  ...
  var promises = Object.values(serializedObject).map(function(value) {
    ...
    if (value.type) {
      return enlivenObjects([value]).then(function (enlived) {
        return enlived[0];
      });
    }
    if (value.source) {
      return Pattern.fromObject(value);
    }
    return value;
  });
 ...
 * @return {CanvasElement} initialized canvas element
 */
function copyCanvasElement(canvas) {
 ...
 */
function toDataURL(canvasEl, format, quality) {
 ...
 * @return {Number[]} transform matrix
 */
function calcDimensionsMatrix(options) {
 ...
 * @return {Number[]} transform matrix
 */
function composeMatrix(options) {
 ...
 */
function parsePreserveAspectRatioAttribute(attribute) {
 ...
 */
function limitDimsByArea(ar, maximumArea) {
  var roughWidth = Math.sqrt(maximumArea * ar),
      perfLimitSizeY = Math.floor(maximumArea / roughWidth);
  return { x: Math.floor(roughWidth), y: perfLimitSizeY };
}

function capValue(min, value, max) {
 ...
 */
function findScaleToFit(source, destination) {
 ...
 */
function findScaleToCover(source, destination) {
...
function removeTransformFromObject(object, transform) {
  var inverted = invertTransform(transform),
...
function applyTransformToObject(object, transform) {
  var options = qrDecompose(transform),
 ...
 */
function sizeAfterTransform(width, height, options) {
...
function mergeClipPaths(c1, c2) {
  ...
  return new Group([a], { clipPath: b, inverted: inverted });
}
