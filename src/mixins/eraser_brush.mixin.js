...
import { PencilBrush } from "../brushes/index.js";
...
var __createBaseClipPathSVGMarkup = fabricObject.prototype._createBaseClipPathSVGMarkup;
...
util.object.extend(fabricObject.prototype, {
  ...
  _createEraserSVGMarkup: function (reviver) {
    if (this.eraser) {
      this.eraser.clipPathId = 'MASK_' + fabricObject.__uid++;
      return [
...
util.object.extend(Group.prototype, {
  ...
  _addEraserPathToObjects: function (path) {
    return Promise.all(this._objects.map(function (object) {
      return EraserBrush.prototype._addPathToObjectEraser.call(
        EraserBrush.prototype,
        object,
        path
      );
    }));
            ...
            .then(function (eraser) {
              ...
              return Promise.all(eraser.getObjects('path')
                .map(function (path) {
                  ...
                  var originalTransform = util.multiplyTransformMatrices(
                    transform,
                    path.calcTransformMatrix()
                  );
                  util.applyTransformToObject(path, originalTransform);
                  return clipPath ?
                    clipPath.clone()
                      .then(function (_clipPath) {
                        var eraserPath = EraserBrush.prototype.applyClipPathToPath.call(
                          EraserBrush.prototype,
                          path,
                          _clipPath,
                          transform
 ...
 * @memberof fabric
 */
const Eraser = util.createClass(Group, {
  /**
   * @readonly
   * @static
   */
 ...
 * @returns {Promise<Eraser>}
 */
Eraser.fromObject = function (object) {
  var objects = object.objects || [],
      options = util.object.clone(object, true);
  delete options.objects;
  return Promise.all([
    util.enlivenObjects(objects),
    util.enlivenObjectEnlivables(options)
  ]).then(function (enlivedProps) {
    return new Eraser(enlivedProps[0], Object.assign(options, enlivedProps[1]), true);
  });
};

var __renderOverlay = Canvas.prototype._renderOverlay;
/**
 * @fires erasing:start
 * @fires erasing:end
 */
util.object.extend(Canvas.prototype, {
  /**
 ...
 * @memberof fabric
 */
const EraserBrush = util.createClass(
  PencilBrush,
  /** @lends EraserBrush.prototype */ {
    type: 'eraser',
    ...
    preparePattern: function (objects) {
      if (!this._patternCanvas) {
        this._patternCanvas = util.createCanvasElement();
      }
    ...
    /**
     * We indicate {@link PencilBrush} to repaint itself if necessary
     * @returns
     */
    ...
    applyClipPathToPath: function (path, clipPath, clipPathContainerTransformMatrix) {
      var pathInvTransform = util.invertTransform(path.calcTransformMatrix()),
      ...
      path.clipPath = path.clipPath ? util.mergeClipPaths(clipPath, path.clipPath) : clipPath;
      return path;
    },
    ...
    _addPathToObjectEraser: function (obj, path, context) {
      ...
      return path.clone()
        .then(function (path) {
          ...
          var desiredTransform = util.multiplyTransformMatrices(
            util.invertTransform(
              obj.calcTransformMatrix()
            ),
            path.calcTransformMatrix()
          );
          util.applyTransformToObject(path, desiredTransform);
          eraser.add(path);
          obj.set('dirty', true);
