...
import * as brushes from './brushes/index.js';
import { filters } from './filters/index.js';
import * as mixins from './mixins/index.js';
import { ActiveSelection, Circle, Ellipse, Group, Image, IText, Line, fabricObject, Path, Polygon, Polyline, Rect, Text, Textbox, Triangle } from './shapes/index.js';
import { version } from './version.js';
import { Canvas } from './canvas.class.js';
import { Color } from './color.class.js';
import { Control } from './control.class.js';
import { controlsUtils } from './controls.actions.js';
import { ElementsParser } from './elements_parser.js';
import { Gradient } from './gradient.class.js';
import { Intersection } from "./intersection.class.js";
...
import { Canvas2dFilterBackend } from './filters/2d_backend.class.js';
...
export let svgValidTagNamesRegEx = getSvgRegex(svgValidTagNames);
export let svgViewBoxElementsRegEx = getSvgRegex(svgViewBoxElements);
export let svgInvalidAncestorsRegEx = getSvgRegex(svgInvalidAncestors);
export let svgValidParentsRegEx = getSvgRegex(svgValidParents);

export let cssRules = { };
export let gradientDefs = { };
export let clipPaths = { };
...
function rotateMatrix(matrix, args) {
 var cos = util.cos(args[0]), sin = util.sin(args[0]),
     x = 0, y = 0;
 if (args.length === 3) {
   x = args[1];
   y = args[2];
...
function skewMatrix(matrix, args, pos) {
 matrix[pos] = Math.tan(util.degreesToRadians(args[0]));
}
...
return function(attributeValue) {
 ...
 attributeValue.replace(reTransform, function(match) {
   ...
   switch (operation) {
     case 'translate':
       ...
       break;
     case 'rotate':
       args[0] = util.degreesToRadians(args[0]);
 ...
 while (matrices.length > 1) {
   matrices.shift();
   combinedMatrix = util.multiplyTransformMatrices(combinedMatrix, matrices[0]);
 }
...
preserveAspectRatio = util.parsePreserveAspectRatioAttribute(preserveAspectRatio);
...
var svgUid =  fabric.Object.__uid++, i, len,
   ...
   descendants = util.toArray(doc.getElementsByTagName('*'));
...
descendants.filter(function(el) {
 ...
 tClipPaths[id] = util.toArray(el.getElementsByTagName('*')).filter(function(el) {
   return svgValidTagNamesRegEx.test(el.nodeName.replace('svg:', ''));
 });
});
gradientDefs[svgUid] = fabric.getGradientDefs(doc);
cssRules[svgUid] = fabric.getCSSRules(doc);
clipPaths[svgUid] = tClipPaths;
