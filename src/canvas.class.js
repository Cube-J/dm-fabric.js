...
import { PencilBrush } from "./brushes/pencil_brush.class.js"
import { controlsUtils } from './controls.actions.js';
import { Point } from "./point.class.js";
import { fabricObject } from './shapes/object.class.js'

var getPointer = util.getPointer,
    degreesToRadians = util.degreesToRadians,
    isTouchEvent = util.isTouchEvent;
 ...
 *
 */
const Canvas = util.createClass(StaticCanvas, /** @lends Canvas.prototype */ {
  ...
  _normalizePointer: function (object, pointer) {
    var m = object.calcTransformMatrix(),
        invertedM = util.invertTransform(m),
        vptPointer = this.restorePointerVpt(pointer);
    return util.transformPoint(vptPointer, invertedM);
  },
  ...
  isTargetTransparent: function (target, x, y) {
    ...
    if (target.shouldCache() && target._cacheCanvas && target !== this._activeObject) {
      ...
      var isTransparent = util.isTransparent(
   ...
   * @param {Event} e Event object
   * @param {Object} [target] inserted back to help overriding. Unused
   */
  ...
  _setupCurrentTransform: function (e, target, alreadySelected) {
    ...
    if (target.group) {
      ...
      pointer = util.transformPoint(pointer, util.invertTransform(target.group.calcTransformMatrix()));
    }
    var corner = target.__corner,
        control = target.controls[corner],
        actionHandler = (alreadySelected && corner) ?
          control.getActionHandler(e, target, control) : controlsUtils.dragHandler,
        ...
        transform = {
          ...
          shiftKey: e.shiftKey,
          altKey: altKey,
          original: util.saveObjectTransform(target),
        };
  ...
  _drawSelection: function (ctx) {
    var selector = this._groupSelector,
        ...
        start = util.transformPoint(viewportStart, this.viewportTransform),
        viewportExtent = new Point(selector.ex + selector.left, selector.ey + selector.top),
        extent = util.transformPoint(viewportExtent, this.viewportTransform),
   ...
   * @return {Object} the target found
   */
   ...
   * @param {Object} [pointer] x,y object of point coordinates we want to check.
   * @return {Object} **top most object from given `objects`** that contains pointer
   * @private
   */
   ...
   * @see {@link Canvas#_searchPossibleTargets}
   * @param {Array} [objects] objects array to look into
   * @param {Object} [pointer] x,y object of point coordinates we want to check.
   * @return {Object} **top most object on screen** that contains pointer
   */
  ...
  _createUpperCanvas: function () {
    ...
    util.addClass(upperCanvasEl, 'upper-canvas ' + lowerCanvasClass);
  ...
  _applyCanvasStyle: function (element) {
    ...
    util.makeElementUnselectable(element);
  },
   ...
   * @param {Object} obj old activeObject
   */
   ...
   * @param {Object} object Object to set as an active one
  ...
  dispose: function () {
    ...
    util.cleanUpJsdomNode(upperCanvasEl);
    this.upperCanvasEl = undefined;
    util.cleanUpJsdomNode(cacheCanvasEl);
   ...
   * @private
   * @param {Object} [instance] the object to transform (gets mutated)
  ...
  _realizeGroupTransformOnObject: function(instance) {
    if (instance.group && instance.group.type === 'activeSelection' && this._activeObject === instance.group) {
      ...
      util.addTransformToObject(instance, this._activeObject.calcOwnMatrix());
