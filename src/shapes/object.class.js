import { util, runningAnimations } from '../util/index.js';
import { CommonMethods } from '../mixins/shared_methods.mixin.js';
import { perfLimitSizeTotal, maxCacheSideLimit, minCacheSideLimit, iMatrix, devicePixelRatio, browserShadowBlurConstant } from "../header.js";
 ...
 * @fires drop
 */
const fabricObject = util.createClass(CommonMethods, /** @lends fabricObject.prototype */ {
  ...
  _limitCacheSize: function(dims) {
    ...
    var ar = width / height, limitedDims = util.limitDimsByArea(ar, perfLimitSizeTotal),
        capValue = util.capValue,
  ...
  _removeDefaultValues: function(object) {
    var prototype = util.getKlass(object.type).prototype;
  ...
  getObjectScaling: function() {
    ...
    var options = util.qrDecompose(this.calcTransformMatrix());
  ...
  getTotalAngle: function () {
    return this.group ?
      util.qrDecompose(this.calcTransformMatrix()).angle :
      this.angle;
  },
  ...
  drawClipPathOnCache: function(ctx, clipPath) {
    ...
    if (clipPath.absolutePositioned) {
      var m = util.invertTransform(this.calcTransformMatrix());
      ctx.transform(m[0], m[1], m[2], m[3], m[4], m[5]);
    }
  ...
  _renderControls: function(ctx, styleOverride) {
    ...
    matrix = util.multiplyTransformMatrices(vpt, matrix);
    options = util.qrDecompose(matrix);
    ctx.save();
  ...
  _applyPatternForTransformedGradient: function(ctx, filler) {
    var dims = this._limitCacheSize(this._getCacheCanvasDimensions()),
        pCanvas = util.createCanvasElement(), pCtx, retinaScaling = this.canvas.getRetinaScaling(),
  ...
  _assignTransformMatrixProps: function() {
    if (this.transformMatrix) {
      var options = util.qrDecompose(this.transformMatrix);
      this.flipX = false;
      this.flipY = false;
  ...
  _removeTransformMatrix: function(preserveAspectRatioOptions) {
    ...
    if (this.transformMatrix) {
      ...
      center = util.transformPoint(center, this.transformMatrix);
    }
  ...
  toCanvasElement: function(options) {
    ...
    var utils = util, origParams = utils.saveObjectTransform(this),
  ...
  toDataURL: function(options) {
    options || (options = { });
    return util.toDataURL(this.toCanvasElement(options), options.format || 'png', options.quality || 1);
  },
...
util.createAccessors && util.createAccessors(fabricObject);

extend(fabricObject.prototype, Observable);
...
/**
 * Defines which properties should be enlivened from the object passed to {@link fabricObject._fromObject}
 * @static
 * @memberOf fabricObject
 * @constant
 * @type string[]
...
fabricObject._fromObject = function(klass, object, extraParam) {
  var serializedObject = clone(object, true);
  return util.enlivenObjectEnlivables(serializedObject).then(function(enlivedMap) {
    var newObject = Object.assign(object, enlivedMap);
    return extraParam ? new klass(object[extraParam], newObject) : new klass(newObject);
  });
};

fabricObject.fromObject = function(object) {
  return fabricObject._fromObject(fabricObject, object);
};
