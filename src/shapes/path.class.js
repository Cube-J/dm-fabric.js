 ...
 */
const Path = util.createClass(fabricObject, /** @lends Path.prototype */ {
  ...
  */
   ...
   */
  _toSVG: function() {
    var path = util.joinPath(this.path);
    return [
      '<path ', 'COMMON_PARTS',
      'd="', path,
  ...
  _calcDimensions: function() {
    ...
    for (var i = 0, len = this.path.length; i < len; ++i) {
      ...
      switch (current[0]) { // first letter
        ...
        case 'C': // bezierCurveTo, absolute
          bounds = util.getBoundsOfCurve(x, y,
            current[1],
            current[2],
            current[3],
            current[4],
 ...
 */
Path.fromObject = function(object) {
  return fabricObject._fromObject(Path, object, 'path');
};
 ...
 */
Path.ATTRIBUTE_NAMES = SHARED_ATTRIBUTES.concat(['d']);
