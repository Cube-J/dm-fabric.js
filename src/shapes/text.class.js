...
import { charWidthsCache, SHARED_ATTRIBUTES } from "../header.js";
import { fabric } from "../index.js";

let _measuringContext;
var clone = util.object.clone;
 ...
 */
const Text = util.createClass(fabricObject, /** @lends Text.prototype */ {
   ...
   * once created it will be referenced on _measuringContext to avoid creating a canvas for every
   * text object created.
  ...
  setPathInfo: function() {
    var path = this.path;
    if (path) {
      path.segmentsInfo = util.getPathSegmentsInfo(path.path);
    }
  },
  ...
  getMeasuringContext: function() {
    ...
    if (!_measuringContext) {
      _measuringContext = this.canvas && this.canvas.contextCache ||
        util.createCanvasElement().getContext('2d');
    }
  ...
  getFontCache: function(decl) {
    ...
    if (!charWidthsCache[fontFamily]) {
      charWidthsCache[fontFamily] = { };
    }
    var cache = charWidthsCache[fontFamily],
  ...
  _measureLine: function(lineIndex) {
    ...
    if (path) {
      ...
      startingPoint = util.getPointOnPath(path.path, 0, path.segmentsInfo);
  ...
  _setGraphemeOnPath: function(positionInPath, graphemeInfo, startingPoint) {
    ...
    var info = util.getPointOnPath(path.path, centerPosition, path.segmentsInfo);
  ...
  _applyPatternGradientTransformText: function(filler) {
    var pCanvas = util.createCanvasElement(), pCtx,
  ...
  graphemeSplit: function (value) {
    return util.string.graphemeSplit(value);
  },
 ...
 */
Text.ATTRIBUTE_NAMES = SHARED_ATTRIBUTES.concat(
...
Text.fromElement = function(element, callback, options) {
  ...
  options = util.object.extend((options ? clone(options) : { }), parsedAttributes);
...
Text.fromObject = function(object) {
  return fabricObject._fromObject(Text, object, 'text');
};

Text.genericFonts = ['sans-serif', 'serif', 'cursive', 'fantasy', 'monospace'];

util.createAccessors && util.createAccessors(Text);
