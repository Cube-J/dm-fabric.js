...
import { Rect } from "./rect.class.js";

var multiplyTransformMatrices = util.multiplyTransformMatrices,
 ...
 */
const Group = util.createClass(fabricObject, Collection, /** @lends Group.prototype */ {
  ...
  canEnter: function (object) {
    if (object === this || this.isDescendantOf(object)) {
      /* _DEV_MODE_START_ */
      console.warn('Group: trying to add group to itself, this call has no effect');
      /* _DEV_MODE_END_ */
      return false;
    }
    else if (object.group && object.group === this) {
      /* _DEV_MODE_START_ */
      console.warn('Group: duplicate objects are not supported inside group, this call has no effect');
      /* _DEV_MODE_END_ */
      return false;
    }
    return true;
  ...
  prepareInitialBoundingBox: function (layoutDirective, objects, context) {
    ...
    if (this.angle) {
      var rad = degreesToRadians(this.angle),
          sin = Math.abs(util.sin(rad)),
          cos = Math.abs(util.cos(rad));
      sizeAfter.setXY(
      ...
      strokeWidthVector = util.rotateVector(strokeWidthVector, rad);
 ...
 */
Group.fromObject = function(object) {
  ...
  return Promise.all([
    ...
    return new Group(enlivened[0], Object.assign(options, enlivened[1]), true);
  });
};
